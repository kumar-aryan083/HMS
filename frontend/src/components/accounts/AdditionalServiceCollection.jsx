...
const AdditionalServiceCollection = ({ data }) => {
  ...
  const totalCashAmount = currentItems
    .filter((item) => item.paymentType?.toLowerCase() === "cash")
    .reduce((sum, obj) => sum + (obj.amount || 0), 0);
  const totalCreditAmount = currentItems
    .filter((item) => item.paymentType?.toLowerCase() === "credit")
    .reduce((sum, obj) => sum + (obj.amount || 0), 0);
  const totalOnlineAmount = totalAmount - totalCashAmount - totalCreditAmount;
  ...
  return (
    <div className="ipd-collections-container">
      ...
      >
        ...
        <strong> Credit:</strong> {totalCreditAmount.toFixed(2)}
      </div>
      {/* <h3 className="ipd-collections-title">Pharmacy Collections</h3> */}
      <table className="ipd-collections-table">
        <thead>
          <tr>
            <th className="ipd-table-header">#</th>
            <th className="ipd-table-header">Date</th>
            {/* <th className="ipd-table-header">Category</th> */}
          ...
          {currentItems?.length > 0 ? (
            currentItems.map((item, index) => (
              <tr key={index} className="ipd-table-row">
                ...
                {/* <td className="ipd-table-cell">{item.category}</td> */}
...
export default AdditionalServiceCollection;
