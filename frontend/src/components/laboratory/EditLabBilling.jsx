...
const EditLabBilling = ({
  ...
  const fetchPatientDetails = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setRailwayType(data.patientDetails.find((patient)=> patient._id === billingDetails.patientId).railwayType);
        setPatientType(data.patientDetails.find((patient)=> patient._id === billingDetails.patientId)?.patientType?.name);
        // console.log("setting patient type",data.patientDetails.find((i)=> i._id === billingDetails?.patientId))
      }
    } catch (error) {
      console.error("Error fetching patient details:", error);
    }
  ...
  const handlePatientSelect = (patient) => {
    ...
    setRailwayType(patient.railwayType);
    setPatientType(patient.patientType.name);
    setFilteredPatients([]);
  };
