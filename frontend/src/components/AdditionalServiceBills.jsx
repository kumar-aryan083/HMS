...
import AddPharmacyBilling from "./pharmacy/AddPharmacyBilling.jsx";
import EditPharmacyBilling from "./pharmacy/EditPharmacyBilling.jsx";
import PharmacyBillingPrint from "./pharmacy/PharmacyBillingPrint.jsx";
...
import AddAdditionalServiceBills from "./AddAdditionalServiceBills.jsx";
import { environment } from "../../utlis/environment.js";
import EditAdditionalServiceBill from "./EditAdditionalServiceBill.jsx";

const AdditionalServiceBills = ({ admissionId }) => {
  ...
  const fetchServiceBillings = async () => {
    setLoading(true);
    try {
      const res = await fetch(
        `${environment.url}/api/additional-services/get-additional-service-bills?startDate=${startDate}&endDate=${endDate}`,
        {
          method: "GET",
          headers: {
  ...
  const handleAddBilling = async (billing) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/additional-services/add-additional-service-bill`,
        {
          method: "POST",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const handleDeleteBilling = async (billingId) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/additional-services/delete-additional-service-bill/${billingId}`,
        {
          method: "DELETE",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const handleEditSubmit = async (bill) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/additional-services/edit-additional-service-bill/${selectedBill._id}`,
        {
          method: "PUT",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(
      workbook,
      worksheet,
      "Additional Services Billings"
    );
    XLSX.writeFile(workbook, "additional_services.xlsx");
  };
  ...
  const exportToCsv = () => {
    ...
    XLSX.writeFile(workbook, "additional_services.csv", { bookType: "csv" });
  };
  ...
  const exportToPdf = () => {
    ...
    const pdf = new jsPDF();
    pdf.text("Additional Service Billings", 14, 10);
    ...
    pdf.save("additional_services.pdf");
  };
  ...
  return (
    <>
      ...
      >
        <h2 style={{ paddingTop: "10px" }}>Additional Service Billings</h2>
...
export default AdditionalServiceBills;
