...
const IpdStatistics = () => {
  ...
  useEffect(() => {
    document.title = "IPD Statistics | HMS";
    if (
      !user ||
      (user.role !== "admin" )
    ) {
  ...
  const fetchStats = async () => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/ipd/get-ipd-stats?startDate=${startDate}&endDate=${endDate}`,
        {
          method: "GET",
          headers: {
  ...
  const chartData1 = {
    labels: ["Total Bills", "Total Credit Patients", "Total Cash Patients"],
    datasets: [
      {
        ...
        data: stats
          ? [
              stats.totalBills,
              stats.totalCreditPatients,
              stats.totalCashPatients,
            ]
          : [],
  ...
  const chartData2 = {
    labels: ["Total Charge", "Total Discount", "Final Amount"],
    datasets: [
      {
        label: "Financial Statistics",
        data: stats
          ? [stats.totalCharge, stats.totalDiscount, stats.totalAmount]
          : [],
