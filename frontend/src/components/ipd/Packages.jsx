...
const Packages = () => {
  ...
  const fetchPackages = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${environment.url}/api/common/get-packages`, {
        method: "GET",
        headers: {
          token: localStorage.getItem("token"),
        },
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/common/edit-package/${id}`,
        {
          method: "PUT",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const handleEditing = (packages) => {
    // console.log(rate);
    editRef.current.style.display = "flex";
    setId(packages._id);
    setForm({
      name: packages.name,
      patientTypes: packages.patientTypes,
      railwayCode: packages.railwayCode,
      nabhPrice: packages.nabhPrice,
      nonNabhPrice: packages.nonNabhPrice,
    });
  };
  ...
  const handleDeletePackage = async (packageId) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/common/delete-package/${packageId}`,
        {
          method: "DELETE",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const handleAddPackage = async (packages) => {
    // console.log("added packages", packages);
    const userDetails = getUserDetails();
    const updatedForm = { ...packages, ...userDetails };
    try {
      const res = await fetch(`${environment.url}/api/common/add-package`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          token: localStorage.getItem("token"),
  ...
  const handleSearch = () => {
    if (searchQuery === "") {
      setFilteredPackages(packages); // If search query is empty, show all wings
    } else {
      const searchResults = packages.filter((packages) =>
        packages.name.toLowerCase().includes(searchQuery.toLowerCase())
      );
      setFilteredPackages(searchResults); // Set filtered wings based on search query
    }
  };
  ...
  const exportToExcel = () => {
    ...
    const data = filteredPackages.map((packages, index) => {
      let row = {
        "#": index + 1,
        "Item Name": packages.name,
      };

      // Populate patient type columns dynamically
      allPatientTypes.forEach((type) => {
        const foundType = packages.patientTypes.find(
      ...
      const updatedRow = {
        ...row,
        "NABH Price": packages.nabhPrice || "-",
        "NON-NABH Price": packages.nonNabhPrice || "-",
      };
      return updatedRow;
    });
  ...
  const exportToCsv = () => {
    ...
    const csvRows = filteredPackages.map((packages, index) => {
      return [
        index + 1,
        packages.name,
  ...
  const exportToPdf = () => {
    ...
    // Prepare table body
    const tableData = filteredPackages.map((packages, index) => {
      return [
        index + 1,
        packages.name,
        ...allPatientTypes.map((type) => {
  ...
  return (
    <>
      <div className="upper-wing">
        ...
        {user?.role === "admin" && (
          <button onClick={() => setIsModalOpen(true)}>Add Operation</button>
        )}
      </div>
      {loading ? (
        <Loader />
                ...
                currentItems.map((packages) => (
                  <tr key={packages._id}>
                    ...
                    <td>
                      ...
                      >
                        {packages.patientTypes.map((type) => (
                          <option
                    ...
                    {/* <td>{packages.railwayCode || "N/A"}</td> */}
                    <td>
                      {packages.patientTypes.find(
                        (type) => type.patientTypeName === "General"
                      )?.generalFees || "-"}
                    </td>
                    {/* <td>{packages.nabhPrice || "N/A"}</td>
                    <td>{packages.nonNabhPrice || "N/A"}</td> */}
                    {user?.role === "admin" && (
                      <td className="wing-btn" style={{ height: "49px" }}>
                        <FontAwesomeIcon
                          icon={faEdit}
                          onClick={() => handleEditing(packages)}
                          title="Edit"
                          className="icon"
                        />
                        <FontAwesomeIcon
