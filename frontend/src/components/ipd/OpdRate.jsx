...
import OpdRateModal from "./OpdRateModal.jsx";
...
const OpdRate = () => {
  ...
  const fetchOpdRates = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${environment.url}/api/common/get-opd-rate`, {
        method: "GET",
        headers: {
          token: localStorage.getItem("token"),
        },
      ...
      if (res.ok) {
        setOpdRates(data.opdRates);
        setFilteredRates(data.opdRates);
      }
    } catch (error) {
      console.log(error);
    } finally {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/common/edit-opd-rate/${id}`,
        {
          method: "PUT",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const handleDeleteRate = async (rateId) => {
    ...
    try {
      const res = await fetch(
        `${environment.url}/api/common/delete-opd-rate/${rateId}`,
        {
          method: "DELETE",
          headers: {
            "x-tenant-id": environment.tenantId,
  ...
  const handleAddOpd = async (opdRate) => {
    // console.log(opdRate);
    const userDetails = getUserDetails();
    const updatedForm = { ...opdRate, ...userDetails };
    try {
      // console.log("handleopd: ", opdRate);
      const res = await fetch(`${environment.url}/api/common/add-opd-rate`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          token: localStorage.getItem("token"),
  ...
  const handleSearch = () => {
    if (searchQuery === "") {
      setFilteredRates(opdRates); // If search query is empty, show all wings
    } else {
      const searchResults = opdRates.filter((rate) =>
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "OPD Rates");

    // Save the file
    XLSX.writeFile(workbook, "opd_rates.xlsx");
  };
  ...
  const exportToCsv = () => {
    ...
    link.setAttribute("download", "opd_rates.csv");
  ...
  const exportToPdf = () => {
    ...
    // Save PDF
    doc.save("opd_rates.pdf");
  };

  return (
    <>
      <div className="upper-wing" style={{ margin: "40px" }}>
        <h2>OPD Rate</h2>
        <div
        ...
        {user?.role === "admin" && (
          <button onClick={() => setIsModalOpen(true)}>Add OPD Rate</button>
        )}
      </div>
      {loading ? (
        <Loader />
      ) : (
        <div className="lower-wing" style={{ margin: "40px" }}>
              ...
              {currentItems.length > 0 ? (
                ...
                <tr>
                  <td colSpan="7" style={{ textAlign: "center" }}>
                    No OPD Rates Available to Show
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <div className="edit-wing" ref={editRef}>
        ...
        >
          ...
          <form onSubmit={handleEditSubmit}>
            ...
            <button type="submit">Update Opd Rate</button>
          </form>
        </div>
      </div>
    </>
